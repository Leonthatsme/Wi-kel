(() => {
  ...
  function initAdmin() {
    ...
    document.getElementById('newProductButton').addEventListener('click', () => dialog.showModal());
    ...
    try {
      ...
      container.innerHTML = products
        .map(product => {
          const [label, state] = statusFor(product);
          return `
            <article class="admin-product" data-id="${product.id}" data-stock="${product.stock}">
              <div class="product-icon" aria-hidden="true">${product.icon || '•'}</div>
      ...
      container.innerHTML = `<p class="load-error">${errorMessage(error)}</p>`;
    }
  }
